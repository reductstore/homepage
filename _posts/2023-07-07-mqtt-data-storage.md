...
This is because MQTT is the protocol that defines what kind of data is transmitted and how it is transmitted. This means that the data storage solution must be able to handle the data in the format that is transmitted with the same speed and efficiency. Or at least, we should be aware of the tradeoffs.

This is what this article is about, we will try to cut through the fog and explore some key factors to consider when selecting the right option, in the following order:
...
An important aspect to consider is the "real-time" aspect of IoT projects. MQTT messages are time-stamped, which means that subscribers can know exactly when a message was published. This is important to consider when choosing a storage solution, as it must be able to handle the high throughput of MQTT messages and store them in a time-series format.

The time-series format is a data model that stores data in chronological order, making it ideal for storing MQTT messages. This is even more important when querying the data, as it allows for efficient retrieval of messages based on their timestamp.

Another aspect to consider is the type of data that can be transmitted via MQTT. Each MQTT message contain a payload which can be up to 256MB formatted as a string. This means that MQTT can be used to transmit a wide variety of data, including text, images, or audio as long as it is formatted as a string.

At the same time, considering that MQTT data may contain any kind of sensitive information like device telemetry, user activities, or images, it is crucial to select a storage option that ensures the security and confidentiality of the data.
...
In storing pictures from a camera, or high-frequency sensor data (e.g. accelerometers) performance becomes even more critical. Cameras often generate high-resolution images that can be pretty large, and an average accelerometer can produce around 4,000 measurements per second. 
...
When it comes to retrieving these stored pictures for analysis or review purposes, speed is crucial. You should be able to easily access and fetch images from any specified time interval.
...
- InfluxDB: an open-source database that provides high-performance storage and retrieval of time-stamped data with a SQL-like query language. You can store numbers (integer or floating point values), boolean values, or text strings (with a limit of 64KB).
...
If you need to install a blob storage on your edge device, you should consider MinIO. It is an open-source, high-performance object storage system designed to store unstructured data such as photos, videos, log files, backups, and container/VM images. 
...
Thanks for reading, I hope this article will help you choose the most appropriate MQTT data storage option to meet your specific needs.